...
cos <- filter(co_cities, first_city == "Colorado Springs")
plot(st_geometry(cos))
...
ggplot(data = filter(zcta_impacts, pol == "pm" & outcome == "mort_ac")) +
...
ggplot(data = filter(zcta_impacts, pol == "pm" & outcome == "hosp_cvd")) +
...
ggplot(data = filter(zcta_impacts, pol == "pm" & outcome == "hosp_res")) +
...
ggplot(data = filter(zcta_impacts, pol == "o3" & outcome == "st_mort_na")) +
...
ggplot(data = filter(zcta_impacts, pol == "o3" & outcome == "hosp_res")) +
...
ggplot(data = zcta_impacts) +
  ggtitle(paste("Persons that are non-Hispanic white alone")) +
  geom_sf(aes(fill = pct_nhw), color = NA) +
  geom_sf(data = cos, fill=NA, color="red") +
  geom_sf(data = pp, color = "black", size = 3) +
  scale_fill_viridis(name = "Percentage") +
  simple_theme +
  theme(legend.position = "bottom",
        axis.text = element_blank(),
        axis.ticks = element_blank()) +
  coord_sf(crs = ll_wgs84, xlim = c(-105.5, -104), ylim = c(38.2, 39.3))
